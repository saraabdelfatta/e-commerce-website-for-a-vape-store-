...
export const Header = ({ sidebarToggle, setSidebarToggle, darkMode, setDarkMode }) => {
    ...
    <header className="sticky top-0 z-99999 flex w-full border-gray-200 bg-white lg:border-b dark:border-gray-800 dark:bg-gray-900">
      <div className="flex grow flex-col items-center justify-between lg:flex-row lg:px-6">
        ...
        <div className="flex w-full items-center justify-between gap-2 border-b border-gray-200 px-3 py-3 sm:gap-4 lg:justify-normal lg:border-b-0 lg:px-0 lg:py-4 dark:border-gray-800">
          {/* Hamburger */}
          <button
            className={`z-99999 flex h-10 w-10 items-center justify-center rounded-lg border-gray-200 text-gray-500 lg:h-11 lg:w-11 lg:border dark:border-gray-800 dark:text-gray-400 ${sidebarToggle ? 'lg:bg-transparent dark:lg:bg-transparent bg-gray-100 dark:bg-gray-800' : ''}`}
            onClick={(e) => { e.stopPropagation(); setSidebarToggle(!sidebarToggle); }}
          >
            {/* Desktop hamburger */}
            ...
            {/* Mobile hamburger */}
            <svg className={`fill-current ${sidebarToggle ? 'hidden' : 'block lg:hidden'}`} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path fillRule="evenodd" clipRule="evenodd" d="M3.25 6C3.25 5.58579 3.58579 5.25 4 5.25L20 5.25C20.4142 5.25 20.75 5.58579 20.75 6C20.75 6.41421 20.4142 6.75 20 6.75L4 6.75C3.58579 6.75 3.25 6.41422 3.25 6ZM3.25 18C3.25 17.5858 3.58579 17.25 4 17.25L20 17.25C20.4142 17.25 20.75 17.5858 20.75 18C20.75 18.4142 20.4142 18.75 20 18.75L4 18.75C3.58579 18.75 3.25 18.4142 3.25 18ZM4 11.25C3.58579 11.25 3.25 11.5858 3.25 12C3.25 12.4142 3.58579 12.75 4 12.75L12 12.75C12.4142 12.75 12.75 12.4142 12.75 12C12.75 11.5858 12.4142 11.25 12 11.25L4 11.25Z" fill="" />
            </svg>
            {/* Close icon for mobile */}
            <svg className={`fill-current ${sidebarToggle ? 'block lg:hidden' : 'hidden'}`} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          {/* Mobile menu toggle */}
          <button
            className={`z-99999 flex h-10 w-10 items-center justify-center rounded-lg text-gray-700 hover:bg-gray-100 lg:hidden dark:text-gray-400 dark:hover:bg-gray-800 ${menuToggle ? 'bg-gray-100 dark:bg-gray-800' : ''}`}
            onClick={(e) => { e.stopPropagation(); setMenuToggle(!menuToggle); }}
          >
          ...
          <div className="hidden lg:block">
            <form onSubmit={(e) => e.preventDefault()}>
              <div className="relative">
                ...
                <input type="text" id="admin-search-input" placeholder="Search products, orders..." className="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-200 bg-transparent py-2.5 pr-14 pl-12 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden xl:w-[430px] dark:border-gray-800 dark:bg-gray-900 dark:bg-white/[0.03] dark:text-white/90 dark:placeholder:text-white/30" />
              </div>
            </form>
          </div>
        </div>

        {/* Nav icons — shown on desktop always, on mobile when menuToggle is true */}
        <div className={`shadow-theme-md w-full items-center justify-between gap-4 px-5 py-4 lg:flex lg:justify-end lg:px-0 lg:shadow-none ${menuToggle ? 'flex' : 'hidden'}`}>
          <div className="2xsm:gap-3 flex items-center gap-2">
            ...
            <div className="relative" ref={notifRef}>
              <button
                id="admin-notif-btn"
                className="hover:text-dark-900 relative flex h-11 w-11 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-500 transition-colors hover:bg-gray-100 hover:text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-white"
                onClick={() => { setNotifOpen(!notifOpen); setNotifying(false); }}
              >
                {notifying && (
              ...
              {notifOpen && (
                <div className="shadow-theme-lg dark:bg-gray-dark absolute -right-[240px] mt-[17px] flex h-[480px] w-[350px] flex-col rounded-2xl border border-gray-200 bg-white p-3 sm:w-[361px] lg:right-0 dark:border-gray-800">
                  <div className="mb-3 flex items-center justify-between border-b border-gray-100 pb-3 dark:border-gray-800">
                    ...
                    <button onClick={() => setNotifOpen(false)} className="text-gray-500 dark:text-gray-400">
                      <svg className="fill-current" width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fillRule="evenodd" clipRule="evenodd" d="M6.21967 7.28131C5.92678 6.98841 5.92678 6.51354 6.21967 6.22065C6.51256 5.92775 6.98744 5.92775 7.28033 6.22065L11.999 10.9393L16.7176 6.22078C17.0105 5.92789 17.4854 5.92788 17.7782 6.22078C18.0711 6.51367 18.0711 6.98855 17.7782 7.28144L13.0597 12L17.7782 16.7186C18.0711 17.0115 18.0711 17.4863 17.7782 17.7792C17.4854 18.0721 17.0105 18.0721 16.7176 17.7792L11.999 13.0607L7.28033 17.7794C6.98744 18.0722 6.51256 18.0722 6.21967 17.7794C5.92678 17.4865 5.92678 17.0116 6.21967 16.7187L10.9384 12L6.21967 7.28131Z" fill="" />
                      </svg>
                    </button>
                  ...
                  <ul className="custom-scrollbar flex h-auto flex-col overflow-y-auto">
                    {[
                      { name: 'Store Order #1042', time: '5 min ago', online: true },
                      { name: 'Stock Alert: Device X', time: '8 min ago', online: true },
                      { name: 'New Customer Signup', time: '15 min ago', online: true },
                    ].map((n, i) => (
                      <li key={i}>
                        <a className="flex gap-3 rounded-lg border-b border-gray-100 p-3 px-4.5 py-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5" href="#">
                          <span className="relative z-1 block h-10 w-full max-w-10 rounded-full">
                            <img src="/admin-assets/images/user-placeholder.svg" alt="User" className="overflow-hidden rounded-full" />
                            <span className={`absolute right-0 bottom-0 z-10 h-2.5 w-full max-w-2.5 rounded-full border-[1.5px] border-white dark:border-gray-900 ${n.online ? 'bg-success-500' : 'bg-error-500'}`}></span>
                          </span>
                          <span className="block">
                            <span className="text-theme-sm mb-1.5 block text-gray-500 dark:text-gray-400">
                              <span className="font-medium text-gray-800 dark:text-white/90">{n.name}</span> requires review
                            </span>
                            <span className="text-theme-xs flex items-center gap-2 text-gray-500 dark:text-gray-400">
                              <span>System</span><span className="h-1 w-1 rounded-full bg-gray-400"></span><span>{n.time}</span>
                            </span>
                          </span>
                        </a>
                      </li>
          ...
          <div className="relative" ref={userRef}>
            <a
              ...
              href="#"
              onClick={(e) => { e.preventDefault(); setUserDropOpen(!userDropOpen); }}
            >
              <span className="mr-3 h-11 w-11 overflow-hidden rounded-full border border-gray-200 dark:border-gray-700">
                <img src="/admin-assets/images/user-placeholder.svg" alt="User" />
              </span>
              <span className="text-theme-sm mr-1 block font-medium">Admin User</span>
              <svg className={`stroke-gray-500 dark:stroke-gray-400 transition-transform ${userDropOpen ? 'rotate-180' : ''}`} width="18" height="20" viewBox="0 0 18 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M4.3125 8.65625L9 13.3437L13.6875 8.65625" stroke="" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </a>
            ...
            {userDropOpen && (
              <div className="shadow-theme-lg dark:bg-gray-dark absolute right-0 mt-[17px] flex w-[260px] flex-col rounded-2xl border border-gray-200 bg-white p-3 dark:border-gray-800">
                <div>
                  <span className="text-theme-sm block font-medium text-gray-700 dark:text-gray-400">Vape Store Admin</span>
                  <span className="text-theme-xs mt-0.5 block text-gray-500 dark:text-gray-400 truncate">{adminEmail}</span>
                </div>
                <ul className="flex flex-col gap-1 border-b border-gray-200 pt-4 pb-3 dark:border-gray-800">
                  <li>
                    <Link to="/admin/profile" className="group text-theme-sm flex items-center gap-3 rounded-lg px-3 py-2 font-medium text-gray-700 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
